...
const ManageItems = () => {
  ...
  const validateSubcategoryForm = () => {
    const { name, description, stockQuantity, rentalPrice, buyPrice } = newSubcategory;
    if (!name || !description || !stockQuantity || !rentalPrice || !buyPrice) {
      toast.error("Please fill in all required fields.", {
        position: "top-right",
        autoClose: 2000,
        transition: Slide,
  ...
  const handleOpenModal = (type, category = null, subcategory = null) => {
    ...
    if (type === "addSubcategory") {
      setNewSubcategory({ name: "", description: "", image: null, stockQuantity: "", isAvailable: false, rentalPrice: "", buyPrice: "" });
